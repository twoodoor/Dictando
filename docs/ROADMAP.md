...
| # | Task | File(s) | Status |
|---|---|---|---|
| B1 | **Two-phase paste** — paste local-cleaned text instantly on key-up; `tokio::spawn` fires cloud call; if result differs, re-inject (select pasted chars + replace) | `lib.rs`, `inject.rs` | `[~]` |
| B2 | **Parallel cloud cleanup** — move `cloud_enhance` from blocking thread to `tokio::spawn`; surface result via channel | `lib.rs`, `ai.rs` | `[~]` |
| B3 | **Hotkey conflict detection** — check `RegisterHotKey` return value, emit `hotkey-conflict` event, show warning badge in Settings | `shortcuts.rs`, `lib.rs`, `SettingsView.tsx` | `[~]` |
